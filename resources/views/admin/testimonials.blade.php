...
<body class="flex flex-wrap" style="background-color: #460C68;">
     ...
     <a href="/">
      <img src="{{ asset('photo/agies-logo.jpg') }}" class="rounded-full h-32 w-32" alt="">
        ...
        <ul class="mt-5">
            ...
            <li class="mb-3 underline decoration-solid"><a href="/admin/testimonials">Testimonials</a></li>
    ...
    <main class="flex flex-1 flex-wrap justify-around content-start gap-5">
        <div class="container px-6 mx-auto grid">
          ...
          <h2
            class="my-3 text-center text-xl text-white">
            You can filter feedbacks here...
          </h2>
...
<div class='flex items-start justify-center my-6'>
    ...
    <div class="flex w-full items-center justify-between border-b pb-3">
      <div class="flex items-center space-x-3">
        <div class="text-lg font-bold text-slate-700">{{ $review->name }}</div>
      </div>
      <div class="flex items-center space-x-8">
        <!-- <button class="rounded-2xl border bg-neutral-100 px-3 py-1 text-xs font-semibold">Category</button> -->
        <div class="text-xs text-neutral-500">{{ \Carbon\Carbon::parse($review->created_at)->format('F d, Y') }}</div>
      </div>
    </div>
    ...
    <div class="mt-4 mb-6">
        ...
        @endif
        <div class="text-xl text-neutral-600">{{ $review->message }}</div>
    </div>
    ...
    <div>
      <div class="flex items-center justify-start text-slate-500">
        ...
        <form action="{{ route('review.remove') }}" method="POST">
          ...
          <button type="submit" class="flex text-base cursor-pointer items-center transition hover:text-slate-600 p-2 bg-red-600 hover:bg-red-700 text-white rounded-xl">
            Delete
          </button>
        </form>
        <form action="{{ route('review.approve') }}" method="POST">
            @csrf
            <input type="hidden" value="{{ $review->id }}" name="id">          
          <button type="submit" class="flex text-base cursor-pointer items-center transition hover:text-slate-600 p-2 bg-green-600 hover:bg-green-700 text-white rounded-xl">
            Approve
          </button>
        </form>
      </div>
